...
- **Multi-Provider Auth:** Supports email+password, Google/Apple/GitHub (via Replit Auth OIDC), and Telegram Login Widget. All providers unified into the `users` table via `authProvider`/`authProviderId` columns. OIDC flow: `/api/login` → Replit OIDC → `/api/callback` → `/api/auth/oidc/complete` (bridges to JWT) → `/login?token=...`. Telegram flow: `POST /api/auth/telegram` validates HMAC-SHA-256 hash and issues JWT. Replit Auth uses a separate `auth_users` table (VARCHAR IDs) and `sessions` table (mandatory, do not drop).
...
- **Pages:**
    ...
    - **Graph View:** D3 force-directed graph with Obsidian-style tiny node dots (1.5-2.5px radius). Features: department galaxy clusters with bounding circles and radial gradients; inter-department blood vessel tubes with bidirectional particles; orbital animation where dependent tasks revolve around upstream tasks; blood flow particles on selected node's full dependency chain (upstream + downstream); 3D node shadows (feDropShadow + radial shine gradient); bottleneck-aware node sizing (downstream dependency count increases radius); due date urgency visuals (overdue=red glow ring+halo 1s pulse, ≤2 days=amber glow 2s pulse) using dedicated SVG glow circles for cross-browser reliability; hover tooltip showing task name/assignee/due date/status/downstream count; full dependency chain highlighting on hover (upstream + downstream, non-chain nodes dimmed); one-click reset-view button; AI analysis button that calls Claude Haiku to identify followUp/important/bottleneck tasks and highlights them with animated glow halos (blue/gold/red); GraphSettings panel for color-by and blood-flow toggles. **Interaction model**: short click = highlight node + full dependency chain (no sheet); long press 400ms = open GraphNodeSheet detail panel; click empty space = deselect all. Nodes stay pinned during interactions (fx/fy held 500ms on click, 1.5s on drag release). Bridge node system: done/cancelled tasks that connect active upstream and downstream nodes are kept as semi-transparent "ghost" nodes (40% opacity, gray fill, dashed ring) with explanatory tooltips; non-bridge completed tasks are filtered out. **Department dragging**: galaxy border circles have invisible drag handles (24px stroke-width) — dragging moves all nodes in that department together, releases after 2s. **Department anti-overlap**: dynamic minimum distance based on actual galaxy radii (radiusA + radiusB + 30), repulsion force 1.5x multiplier. **Performance**: graph-view.tsx uses useMemo([graphData]) to prevent force simulation rebuild on unrelated state changes. **Mobile**: user-select:none + touch-action:none + preventDefault on drag to prevent blue selection overlay.
...
- **GraphChatFloat:** Floating AI chat panel on graph view. Features screenshot capture: camera button captures current SVG graph as PNG via Canvas, sends as base64 image attachment to Claude for visual analysis. AI can see the graph layout and analyze node positions, clusters, and visual patterns. **Conversation persistence:** Automatically creates a conversation record on first message and persists all messages (user, assistant, system/confirm results) to the `conversations`/`chat_messages` tables, so graph AI chats appear in the Chats history page.
- **Universal AI Chat Persistence Rule:** All AI chat interactions anywhere in the app (agent page, graph float, any future chat surfaces) MUST persist to the conversations/chat_messages tables so they appear in Chats history.
...
- **Action Schemas:** Zod schemas define available AI actions (e.g., `create_task`, `update_task`, `query_tasks`, `create_project`, `add_comment`). `create_task` supports `ref`, `dependsOn`, `dependsOnRef` for batch dependency resolution. `update_task` supports `version` for optimistic locking.
- **Action Executor:** Processes and executes confirmed AI actions, logging their source as `ai_chat`. Supports `executeBatchActions()` for transactional batch creation with ref-to-ID dependency resolution and duplicate detection. AI-created tasks now trigger team notifications.
- **Batch Confirm API:** `POST /api/ai/confirm-batch` accepts `{actions: [{actionType, data}...]}` for atomic batch execution. Returns per-action results with `duplicateWarning` fields.
- **Optimistic Locking:** `tasks` table has `version` column (integer, default 1). `updateTaskWithVersion(id, version, data)` enforces version match; returns 409 on conflict ("该任务已被其他人修改，请刷新后重试").
- **Duplicate Detection:** `checkDuplicateTask(orgId, title, assigneeId?)` checks for same-title tasks created within 10 minutes.
- **Meeting Minutes Flow:** AI prompts enforce a two-step process: (1) AI extracts and displays task table for user review, (2) user confirms → AI outputs `multi_confirm` with `ref`/`dependsOnRef` for inter-task dependencies.
