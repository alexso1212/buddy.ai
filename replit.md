...
- **Interactive Action Sheets:** AI confirmation/decision interactions use Bottom Sheet components instead of inline chat cards. `ActionSheet.tsx` provides the reusable wrapper; `MultiConfirmSheet`, `SingleConfirmSheet`, and `DecisionSheet` handle batch confirm, single confirm, and decision resolution respectively. Chat shows summary lines with tap-to-open behavior. Components: `client/src/components/ai/ActionSheet.tsx`, `MultiConfirmSheet.tsx`, `SingleConfirmSheet.tsx`, `DecisionSheet.tsx`. Wired through `BuddyCallbacks.onSendMessage` for decision responses.
