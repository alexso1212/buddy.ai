...
- **Knowledge Base Pipeline:** Supports file upload, text extraction from various formats, intelligent chunking, embedding generation (with ILIKE keyword matching as fallback if embedding API is unavailable), and storage.
...
- **Smart Setup (智能初始化):** Admin-only page (`/setup`) for AI-powered enterprise initialization. Supports 13 file formats (PDF/DOCX/DOC/TXT/MD/XLSX/XLS/CSV/PPTX/HTML/RTF/JSON/ZIP). **3-Stage Analysis Funnel:** Stage 0 (filename filter), Stage 1 (Haiku two-dimension scoring: orgRelevance + kbRelevance + sensitivity on 500 chars), Stage 1.5 (orgRelevance>=3 filter, 15 file / 80K char cap), Stage 2 (Opus deep synthesis). **17-Category Taxonomy:** org_chart/roster/jd/contract/kpi/policy/handbook/sop/product/sales/project/finance/legal/marketing/brand/technical/general. Confirm page groups files by tier with sensitivity badges. Sensitivity=high auto-sets admin visibility on KB import.
