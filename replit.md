...
- **AI Chat UX Enhancements:** Collapsible ThinkingBlock for Claude extended thinking display; token usage badges showing CNY cost (¥X.XX) with token detail tooltips; 50ms token buffering for smoother streaming; 45s timeout detection; error classification with retry buttons; spinner animations on confirm cards; post-action context injection into conversation history; Ctrl+V paste and drag-drop file/image support in input bar; message timestamps (HH:MM); hover-to-reveal AI reply actions; code block syntax highlighting (highlight.js) with line numbers and collapsible long blocks (>20 lines); smart suggestion cards with time-aware context (overdue/today/this week); message edit confirmation warning for mid-conversation edits with "(已编辑)" badge; Web Search toggle persisted to localStorage.
- **Stream Abort Safety:** Proper cleanup on conversation switch (abort old stream before loading new), component unmount, page refresh (beforeunload), and GraphChatFloat panel close. Server-side AbortController signal passed through to AI SDK to stop token generation when client disconnects.
