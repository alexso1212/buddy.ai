...
**Core Data Model:**
A 24-table PostgreSQL database schema manages entities like `organizations`, `departments`, `users`, `projects`, `tasks`, `task_deliverables`, `task_submissions`, `activity_logs`, `notifications`, `conversations`, `chat_messages`, `token_usage`, `user_memories`, `kb_documents`, `kb_chunks`, and `briefings`. It includes features for invitation management, task deliverable versioning, submission tracking, a knowledge base with pgvector (v0.8.0) for semantic search via 1536-dim embeddings, and daily AI briefing generation with per-user-per-day caching.
