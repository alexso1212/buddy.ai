...
- 2026-02-21: V4.4 Mindmap Org View - Modular component architecture (components/org/*), semantic color system (completion rate → border green/blue/amber/red/gray, workload → bg tint, urgency → dots), Xmind-style top-down layout with SVG rounded polyline connectors, fold/unfold with child count badges, focus mode (long-press dims non-branch nodes), zoom/pan canvas (Ctrl+scroll, drag), double-click to fit, OrgMobileTree (vertical indented layout), person popovers with stats/actions, color legend bar, /api/users/stats endpoint
