...
**Core Data Model:**
A 25-table PostgreSQL database schema manages entities like `organizations`, `departments`, `users`, `projects`, `tasks`, `task_deliverables`, `task_submissions`, `activity_logs`, `notifications`, `conversations`, `chat_messages`, `token_usage`, `user_memories`, `kb_documents`, `kb_chunks`, `briefings`, and `member_profiles`. It includes features for invitation management, task deliverable versioning, submission tracking, a knowledge base with pgvector (v0.8.0) for semantic search via 1536-dim embeddings, daily AI briefing generation with per-user-per-day caching, and AI-extracted member profiles with claim mechanism.
...
- **Daily Briefing Service:** AI-generated daily briefings cached per user per day. Services in `server/services/briefing/` (dataAggregator, briefingGenerator). Aggregates tasks (due today, overdue, completed yesterday), team stats, and KB data. Uses Haiku model for generation with fallback template. Routes: `GET /api/briefing/today` (returns cached or generates new), `POST /api/briefing/refresh` (force regeneration). Dashboard displays briefing card with Markdown rendering and refresh button.
...
- **Knowledge Base Page:** Admin-only management page (`/knowledge-base`) for document upload (PDF/DOCX/TXT/MD), document list with status monitoring (pending/processing/ready/error), chunk preview, category/status filtering, reprocessing, and deletion. Auto-refreshes every 5 seconds. Sidebar entry visible only to owner/admin roles under "企业管理" section.
...
- **Smart Setup (智能初始化):** Admin-only page (`/setup`) for AI-powered enterprise initialization. Upload org structure documents (PDF/DOCX/TXT/MD/ZIP) or select existing KB documents for analysis. AI extracts departments, job roles, members, and file classifications via `server/services/setup/`. 4-step wizard: Upload → AI Analysis → Confirm/Edit → Complete. Creates departments, job roles, member profiles, and imports documents to KB in one click. Upload step features a toggle between "上传新文件" (upload new files) and "从知识库选择" (select from KB) modes. KB mode lists completed/ready KB documents with checkboxes for multi-select. **3-Stage Analysis Funnel:** Stage 0 (filename-based skip filter for invoices/receipts/photos), Stage 1 (Haiku relevance scoring 1-5 on first 500 chars), Stage 1.5 (score≥3 filter, cap 15 files / 80K chars), Stage 2 (Opus deep synthesis on selected files only). Reduces cost 60-80% and ensures all file categories (including 'manual') reach Opus. Returns `analyzedFileCount`/`totalFileCount` for frontend display. Routes: `POST /api/setup/analyze`, `POST /api/setup/analyze-kb`, `POST /api/setup/confirm`. Sidebar entry under "企业管理" section (owner/admin only).
- **Member Profiles (成员档案):** AI-extracted or manually created employee profiles with claim mechanism. `member_profiles` table stores pending profiles separate from registered users. Tasks can be assigned to profiles via `tasks.memberProfileId`. When users register and join the org, they can claim their profile (auto-matched by email/name/alias). Admin can also manually bind profiles. Claim process: updates user dept/role (both `users` and `org_memberships` tables), migrates tasks, marks profile as claimed. Routes: `/api/member-profiles` (CRUD), `/api/member-profiles/:id/claim`, `/api/member-profiles/:id/bind/:userId`, `/api/member-profiles/match`. AI context merges registered users + pending profiles in team member list, using `assigneeId` vs `memberProfileId` fields for task assignment. Team page shows two sections: registered members + pending profiles with admin bind/delete controls.
...
| Smart Setup synthesis | Opus | 8000 | off | 0.2 |
