...
- **AI Chat Components:** Features a floating chat button, main chat panel, various message bubble types, action confirmation cards, and an input bar with enhancements like ThinkingBlock (shimmer bar animation + collapsible thinking process), token usage badges, classified error handling, token buffering, file/image support, code block syntax highlighting with line numbers and long-code collapse (30+ lines), smart suggestion cards, and keyboard shortcuts. Streaming responses show a brand-colored blinking cursor and a centered "停止生成" pill button. User message bubbles use warm rgba(174,86,48,0.12) background with asymmetric border-radius. Long AI messages (2000+ chars) auto-collapse with gradient fade and "显示更多" button. External links display an ExternalLink icon.
