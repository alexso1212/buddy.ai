...
- Features model-centric AI Provider Management: models (Claude Opus/Sonnet/Haiku, GPT-4o, DeepSeek) are fixed categories; under each model, admins add/manage multiple API endpoints (base URL optional for official APIs, timeout). Supports two key modes: direct API key storage in DB (`api_key` column) or environment variable reference (`api_key_env_var`). Drag-to-reorder sets per-model priority for fallback. Uses `ai_model_providers` table (new) alongside legacy `ai_providers` table. API keys are masked in GET responses (first 8 + last 4 chars shown).
