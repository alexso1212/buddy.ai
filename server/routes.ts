...
export async function registerRoutes(server: Server, app: Express) {
  ...
  app.get("/api/graph/hierarchy", async (req, res) => {
    try {
      ...
      function computeAggregatedStatus(taskList: typeof allTasks) {
        const total = taskList.length;
        const done = taskList.filter(t => t.status === 'done' || t.status === 'cancelled').length;
        const overdue = taskList.filter(t => t.dueDate && t.dueDate < now && t.status !== 'done' && t.status !== 'cancelled').length;
        const blocked = taskList.filter(t => t.status === 'blocked').length;
        const completionRate = total > 0 ? done / total : 0;
        const hasUrgent = overdue > 0 || blocked > 0;
        return { total, overdue, blocked, completionRate, hasUrgent, done };
      }
      ...
      interface HierarchyNode {
        ...
        aggregatedStatus: ReturnType<typeof computeAggregatedStatus>;
        color: string;
        deptId: number | null;
      ...
      let links: { source: string; target: string; type: string; isBlocking: boolean }[] = [];
      let parentTitle: string | null = null;

      if (level === 0) {
        nodes = allDepartments.map((dept, idx) => {
          const deptProjects = allProjects.filter(p => p.deptId === dept.id);
          const deptProjectIds = new Set(deptProjects.map(p => p.id));
          const deptTasks = allTasks.filter(t => deptProjectIds.has(t.projectId));
          const agg = computeAggregatedStatus(deptTasks);
          return {
            id: `dept-${dept.id}`,
            numericId: dept.id,
            title: dept.name,
            type: 'department' as const,
            status: agg.completionRate === 1 ? 'done' : agg.hasUrgent ? 'at_risk' : 'active',
            childrenCount: deptProjects.length,
            aggregatedStatus: agg,
            color: dept.color || projectColors[idx % projectColors.length],
            deptId: dept.id,
            projectId: null,
            assigneeName: null,
            dueDate: null,
            ...
            priority: 'medium',
            weight: 5,
            progress: Math.round(agg.completionRate * 100),
          };
        });
      } else if (level === 1 && parentId !== null) {
        const dept = allDepartments.find(d => d.id === parentId);
        parentTitle = dept?.name ?? null;
        const deptProjects = allProjects.filter(p => p.deptId === parentId);
        nodes = deptProjects.map((proj, idx) => {
          const projTasks = allTasks.filter(t => t.projectId === proj.id);
          const agg = computeAggregatedStatus(projTasks);
          return {
            id: `proj-${proj.id}`,
            numericId: proj.id,
            title: proj.name,
            ...
            status: proj.status,
            childrenCount: projTasks.filter(t => t.parentTaskId === null).length,
            aggregatedStatus: agg,
            color: projectColors[idx % projectColors.length],
            deptId: parentId,
            projectId: proj.id,
            assigneeName: null,
            dueDate: proj.targetDate ? proj.targetDate.toISOString() : null,
            isOverdue: !!(proj.targetDate && proj.targetDate < now && proj.status !== 'completed'),
            priority: 'medium',
            weight: 5,
            progress: Math.round(agg.completionRate * 100),
          };
        ...
        const proj = allProjects.find(p => p.id === parentId);
        parentTitle = proj?.name ?? null;
        const projTasks = allTasks.filter(t => t.projectId === parentId && t.parentTaskId === null);
        const projAllTasks = allTasks.filter(t => t.projectId === parentId);
        const tasksWithSubtasks = new Set(projAllTasks.filter(t => t.parentTaskId !== null).map(t => t.parentTaskId));
        const taskIds = new Set(projTasks.map(t => t.id));
        const taskStatusMap = new Map(allTasks.map(t => [t.id, t.status]));

        nodes = projTasks.map(t => {
          const assignee = t.assigneeId ? userMap.get(t.assigneeId) : null;
          const subtasks = allTasks.filter(st => st.parentTaskId === t.id);
          const agg = computeAggregatedStatus(subtasks);
          return {
            id: `task-${t.id}`,
            numericId: t.id,
            title: t.title,
            ...
            aggregatedStatus: agg,
            color: proj?.deptId ? (allDepartments.find(d => d.id === proj.deptId)?.color || '#9ca3af') : '#9ca3af',
            deptId: proj?.deptId ?? null,
            projectId: parentId,
            assigneeName: assignee?.displayName ?? null,
            dueDate: t.dueDate ? t.dueDate.toISOString() : null,
            isOverdue: !!(t.dueDate && t.dueDate < now && t.status !== 'done' && t.status !== 'cancelled'),
        ...
        links = allDeps
          ...
          .map(d => ({
            source: `task-${d.dependsOnTaskId}`,
            target: `task-${d.taskId}`,
            type: d.type,
            isBlocking: taskStatusMap.get(d.dependsOnTaskId) !== 'done',
          }));
      } else if (level >= 3 && parentId !== null) {
        const parentTask = allTasks.find(t => t.id === parentId);
        parentTitle = parentTask?.title ?? null;
        const subtasks = allTasks.filter(t => t.parentTaskId === parentId);
        const subtaskIds = new Set(subtasks.map(t => t.id));
        const taskStatusMap = new Map(allTasks.map(t => [t.id, t.status]));
        const deepSubtasks = allTasks.filter(t => t.parentTaskId !== null);
        const tasksWithChildren = new Set(deepSubtasks.map(t => t.parentTaskId));

        nodes = subtasks.map(t => {
          const assignee = t.assigneeId ? userMap.get(t.assigneeId) : null;
          const children = allTasks.filter(st => st.parentTaskId === t.id);
          const agg = computeAggregatedStatus(children);
          return {
            id: `task-${t.id}`,
            numericId: t.id,
            title: t.title,
      ...
      return res.json({ data: { nodes, links, level, parentId, parentTitle } });
    } catch (e: any) {
      return res.status(500).json({ error: e.message });
    }
  });
